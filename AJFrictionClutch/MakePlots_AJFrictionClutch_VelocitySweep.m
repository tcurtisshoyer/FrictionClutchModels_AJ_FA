...
SimulationDirectory = 'G:\AJ_FA_ClutchModels_Demo\AJ_FrictionClutch_VelocitySweep';
...
%% [SUMMARY FIGURE] AJ Friction Clutch Velocity Sweep, 5 Diff Vcl Fractions 
% Plot: 
% Linkage Lifetime
...
print(f,fullfile(PlotOutDir,['SummaryPlots_AJFrictionClutch.png']),'-dpng',['-r' num2str(Resolution_FigurePanel_png)],'-painters');
print(f,fullfile(PlotOutDir,['SummaryPlots_AJFrictionClutch.svg']),'-dsvg','-painters');
...
plotTimeInterval = [930 960]; % Arbitrary 30 second interval
...
for simID = simIDsToPlot  
    ...
    depVarsToPlot = {'Total_Force','numEngaged','sum_Bond_aCatFactin_StrongBound_overAllBonds','sum_Bond_VclFactin_StrongBound_overAllBonds'};
    depVarsLabels = {'Total Force [pN]', '# Engaged Links', '# aCat:Factin in Strong State', '# Vcl:Factin in Strong State'};
