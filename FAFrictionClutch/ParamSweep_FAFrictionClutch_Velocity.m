% This script perfoms a set of simulations with the FA Friction Clutch
...
% commented function "runSimulation_FAClutch.m" for complete documentation.
%
...
%% Simulation Params (see runSimulation_FAClutch.m for complete details)
...
%% Base Model Parameters (see runSimulation_FAClutch.m for complete details)
...
BaseParams.springConstExternal = [1E6]; % Kext [pN/nm] {Arbitrarily high to mimic FN-coated glass substrate}
...
%       See "DefineForceDepRateConstantModels_FA.m" for definition of
%       transition rate constant models.
DefineForceDepRateConstantModels_FA;
% Intg-FN Bond
BaseParams.RateConstants{1} = Intg_alphaVbeta3_FN_BellModelWithMaxLifetimeLimitParams;
% TalinABS3-Factin Bond
BaseParams.RateConstants{2} = TalinABS3_Factin_NegPull_BellModelWithMaxLifetimeLimitParams;
% Vcl-Factin Bond
BaseParams.RateConstants{3} = Vcl_Factin_NegPull_TwoBoundStateParams;
% Vcl-Talin Bond
BaseParams.RateConstants{4} = Vcl_Talin_BellModelWithMaxLifetimeLimitParams;

% Binding Rate Constants
BaseParams.RateConstants{1}.kon = [2]; % on rate for Intg_FN. [1/s]
BaseParams.RateConstants{2}.kon = [2]; % on rate for Tal_Factin. [1/s]
...
BaseParams.RateConstants{4}.kon = [1]; % on rate for Vcl_Tal. [1/s]
...
%           Bond_VclFactin: State of Vcl:Factin bond. 0=unbound, 1=bound.
%           Bond_VclTal: State of Vcl:Tal bond. 0=unbound, 1=bound.
BaseParams.InitialStates.Bond_IntgFN = [0 0 0];
BaseParams.InitialStates.Bond_TalFactin = [0 0 0];
BaseParams.InitialStates.Bond_VclFactin = [0 0 0];
BaseParams.InitialStates.Bond_VclTal = [0 0 0];
...
for V0 = V0_Sweep % Velocity Sweep
    ...
    LinkConfigSweep_singleBondOnRateConstant_Vcl_Tal_Sweep = [0 0 1 1 1 1 1];
    ...
    for linkConfigSweepID = 1:length(LinkConfigSweep_numTotalLinkages)
        ...
        ModelParams.singleBondOnRateConstant_Vcl_Tal = LinkConfigSweep_singleBondOnRateConstant_Vcl_Tal_Sweep(linkConfigSweepID);
        ...
        if isempty(fileCheck)
            tic;
            [DataLog_ClutchEnsemble,DataLog_SingleLinkage,DataLog_Transitions,AllVariableNames,SimulationPerformanceStats] = runSimulation_FAClutch(ModelParams,SimulationParams);
            t=toc;
