function [DataLog_ClutchEnsemble,DataLog_SingleLinkage,DataLog_Transitions,AllVariableNames,SimulationPerformanceStats] = runSimulation_AJClutch(ModelParams,SimulationParams)

%% DOCUMENTATION
% runSimulation_AJClutch: A function that simulates the AJ friction clutch
% model.
%
%
% ----- INPUTS -----
...
VariableNames_DataLog_Transitions = {'t',... % time
    ...
    'idStateVariable',... % ID specifying which state variable undergoes state transition (Bond_CadCad = 1, Bond_aCatFactin = 2, Bond_VclFactin = 3, Bond_VclaCat = 4)
...
VariableNames_DataLog_SingleLinkage = {'t',... % time
        ...
        'Bond_CadCad',... % status of Cad:Cad bond (0=unbound, 1=bound)
        'Bond_aCatFactin',... % status of aCat:Factin bond (0=unbound, 1=bound weak, 2=bound strong)
        'Bond_VclFactin',... % status of Vcl:Factin bond (0=unbound, 1=bound weak, 2=bound strong)
        'Bond_VclaCat',... % status of Vcl:aCat bond (0=unbound, 1=bound)
        'F_Bond_CadCad',... % force across Cad:Cad bond
        'F_Bond_aCatFactin',... % force across aCat:Factin bond
        'F_Bond_VclFactin',... % force across Vcl:Factin bond
        'F_Bond_VclaCat',... % force across Vcl:aCat bond
        };
...
VariableNames_DataLog_ClutchEnsemble = {'t',... % time
        ...
        'mean_F_Bond_CadCad_overAllBonds',... % mean force across all Cad:Cad bonds
        'mean_F_Bond_aCatFactin_overAllBonds',... % mean force across all aCat:Factin bonds
        'mean_F_Bond_VclFactin_overAllBonds',... % mean force across all Vcl:Factin bonds
        'mean_F_Bond_VclaCat_overAllBonds',... % mean force across all Vcl:aCat bonds
        'sum_Bond_CadCad_Bound_overAllBonds',... % # Cad:Cad in bound state (1) across all linkages
        'sum_Bond_aCatFactin_WeakBound_overAllBonds',... % # aCat:Factin in weak bound state (1) across all linkages
        'sum_Bond_aCatFactin_StrongBound_overAllBonds',... % # aCat:Factin in strong bound state (2) across all linkages
        ...
        'sum_Bond_VclaCat_Bound_overAllBonds',... % # Vcl:aCat in bound state (1) across all linkages
        };
...
%       See "DefineForceDepRateConstantModels.m" for definition of
%       transition rate constant models.
RateConstants_Bond_CadCad = ModelParams.RateConstants{1}; 
RateConstants_Bond_aCatFactin = ModelParams.RateConstants{2};
RateConstants_Bond_VclFactin = ModelParams.RateConstants{3};
RateConstants_Bond_VclaCat = ModelParams.RateConstants{4};
...
%           Bond_VclFactin: State of Vcl:Factin bond. 0=unbound, 1=bound.
%           Bond_VclaCat: State of Vcl:aCat bond. 0=unbound, 1=bound.
initState_Bond_CadCad = ModelParams.InitialStates.Bond_CadCad;
initState_Bond_aCatFactin = ModelParams.InitialStates.Bond_aCatFactin;
initState_Bond_VclFactin = ModelParams.InitialStates.Bond_VclFactin;
initState_Bond_VclaCat = ModelParams.InitialStates.Bond_VclaCat;
...
for ii = 1:numLinkagesTotal
    ...
    Bond_CadCad(ii,:) = initState_Bond_CadCad(thisLinkType); % stores state of Cad:Cad bond for each linkage (0=unbound,1=bound)
    Bond_aCatFactin(ii,:) = initState_Bond_aCatFactin(thisLinkType); % stores state of aCat:Factin bond for each linkage (0=unbound, 1=weak_bound, 2=strong_bound)
    ...
    Bond_VclaCat(ii,:) = initState_Bond_VclaCat(thisLinkType); % stores state of Vcl:aCat bond for each linkage (0=unbound,1=bound)
end
F_Bond_CadCad = zeros(numLinkagesTotal,1); % stores force on Cad:Cad bond for each linkage
F_Bond_aCatFactin = zeros(numLinkagesTotal,1); % stores force on aCat:Factin bond for each linkage
F_Bond_VclFactin = zeros(numLinkagesTotal,1); % stores force on Vcl:Factin bond for each linakge
F_Bond_VclaCat = zeros(numLinkagesTotal,1); % stores force on Vcl:aCat bond for each linkage

% Compute Engaged Status (all linkages should be disengaged to start)
Engaged = Bond_CadCad & (Bond_aCatFactin | (Bond_VclFactin & Bond_VclaCat)); % stores engaged status of each linkage (0=disengaged,1=engaged) 
...
while t<tend
    ...
    for thisLinkID = 1:numLinkagesTotal
        ...
        thesePossibleTransitions = getTransitions_1BoundStateModel_Dudko(F_Bond_CadCad(thisLinkID),Bond_CadCad(thisLinkID),RateConstants_Bond_CadCad);
        ...
        % Check aCat_Factin (applies only to Linkage Types 1 and 3)
        if Link_Types(thisLinkID) == 1 || Link_Types(thisLinkID) == 3 
            thisStateVariableID = 2;
            thesePossibleTransitions = getTransitions_2BoundStateModel(F_Bond_aCatFactin(thisLinkID),Bond_aCatFactin(thisLinkID),RateConstants_Bond_aCatFactin);
            allTransitions(count+1:count+size(thesePossibleTransitions,1),:) = ...
                [repmat([thisLinkID thisStateVariableID],[size(thesePossibleTransitions,1) 1]) thesePossibleTransitions];
            count = count+size(thesePossibleTransitions,1);          
        end
        % Check Vcl_Factin and Vcl_aCat (applies only to Linkage Types 2 and 3)
        if Link_Types(thisLinkID) == 2 || Link_Types(thisLinkID) == 3 
            ...
            thesePossibleTransitions = getTransitions_1BoundStateModel_BellwUpperLimit(F_Bond_VclaCat(thisLinkID),Bond_VclaCat(thisLinkID),RateConstants_Bond_VclaCat);
            allTransitions(count+1:count+size(thesePossibleTransitions,1),:) = ...
                [repmat([thisLinkID thisStateVariableID],[size(thesePossibleTransitions,1) 1]) thesePossibleTransitions];
            count = count+size(thesePossibleTransitions,1);
        end          
    ...
    % Then execute the state transition and advance time by dt from Gillespie SSA    
        ...
        if thisStateVariableID==1
            Bond_CadCad(thisLinkID) = thisStateVariableValue;         
        elseif thisStateVariableID==2
            Bond_aCatFactin(thisLinkID) = thisStateVariableValue;
        elseif thisStateVariableID==3
            Bond_VclFactin(thisLinkID) = thisStateVariableValue;
        elseif thisStateVariableID==4
            Bond_VclaCat(thisLinkID) = thisStateVariableValue;
            % If Vh unbinds aCat, Vt forced to unbind F-actin
            if Bond_VclaCat(thisLinkID)==0
    ...
    EngagedAfterTransition = Bond_CadCad(thisLinkID) &&(Bond_aCatFactin(thisLinkID) || (Bond_VclFactin(thisLinkID)&&Bond_VclaCat(thisLinkID)));
    ...
    Engaged = Bond_CadCad & (Bond_aCatFactin | (Bond_VclFactin & Bond_VclaCat));
    ...
    if ~Engaged(thisLinkID)
        F_Bond_CadCad(thisLinkID) = 0;
        F_Bond_aCatFactin(thisLinkID) = 0;
        F_Bond_VclFactin(thisLinkID) = 0;
        F_Bond_VclaCat(thisLinkID) = 0;
        
    % Else (if linkage is engaged)
    else
        % Ecad_Ecad always takes full force
        F_Bond_CadCad(thisLinkID) = Link_Force(thisLinkID);
        
        % Force across adapters depends on which bonds are bound
        % Because this else statement deals only with engaged linkages,
        % this means that either (1) aCat only, (2) Vcl only, or (3) both
        % aCat and Vcl form force-bearing link to F-actin. For aCat to
        % form force-bearing link, must have
        % Bond_aCatFactin(thisLinkID)~=0. For Vcl to form force-bearing
        % link, must have (Bond_VclFactin(thisLinkID)~=0 &&
        % Bond_VclaCat(thisLinkID)~=0).
        % Case 1 for Adapters: aCat is only force-bearing link to F-actin
        if ~(Bond_VclFactin(thisLinkID) && Bond_VclaCat(thisLinkID)) % Already confirmed whole linkage engaged, so this means aCat is only force-bearing link to F-actin
            F_Bond_aCatFactin(thisLinkID) = Link_Force(thisLinkID);
            F_Bond_VclFactin(thisLinkID) = 0;
            F_Bond_VclaCat(thisLinkID) = 0;
        % Case 2 for Adapters: Vcl is only force-bearing link to F-actin
        elseif ~Bond_aCatFactin(thisLinkID) % Already confirmed linkage engaged, so this means Vcl is only force-bearing link to F-actin
            F_Bond_aCatFactin(thisLinkID) = 0;
            F_Bond_VclFactin(thisLinkID) = Link_Force(thisLinkID);
            F_Bond_VclaCat(thisLinkID) = Link_Force(thisLinkID);   
        % Case 3 for Adapters: Both aCat and Vcl form force-bearing links to F-actin
        % Then: Force split equally across two bonds
        else % Already confirmed link engaged, so this means both aCat and Vcl form force-bearing links to F-actin
            F_Bond_aCatFactin(thisLinkID) = Link_Force(thisLinkID)/2;
            F_Bond_VclFactin(thisLinkID) = Link_Force(thisLinkID)/2;
            F_Bond_VclaCat(thisLinkID) = Link_Force(thisLinkID)/2;   
        end
    end
    end
    ...
    if (t-t_lastDataLogLinkages) >= interval_LogLinkageData
        ...
        DataLog_ClutchEnsemble(iDataLog_Linkages,:) = [...
                    ...
                    sum(Link_Force),...
                    mean(F_Bond_CadCad),...
                    mean(F_Bond_aCatFactin),...
                    mean(F_Bond_VclFactin),...
                    mean(F_Bond_VclaCat),...
                    sum(Bond_CadCad==1),...
                    sum(Bond_aCatFactin==1),...
                    sum(Bond_aCatFactin==2),...
